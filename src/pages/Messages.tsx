...
function longDateLabel(raw: string): string {
  const d = parseUTC(raw);
  if (isNaN(d.getTime())) return "";
...
export default function Messages() {
  ...
  const fetchInbox = useCallback(async () => {
    try {
      ...
      res.data.forEach((msg) => {
        const other =
          msg.senderUsername === name ? msg.receiverUsername : msg.senderUsername;
        const existing = map[other];
        if (!existing || parseUTC(msg.sentAt) > parseUTC(existing.sentAt)) {
          map[other] = msg;
        }
      });
      setInboxMap(map);
  ...
  useEffect(() => {
    if (chatTarget?.type === "broadcast" && role === "OWNER") {
      fetchBroadcasts();
    }
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribe(`/user/queue/messages`, (newMsg: Message) => {
      ...
      setInboxMap((prev) => {
        ...
        if (existing && parseUTC(existing.sentAt) >= parseUTC(newMsg.sentAt)) return prev;
  ...
  const getAvatar = (username: string) => {
    ...
    return {
      bg: colors[username.charCodeAt(0) % colors.length],
    ...
    <div className="msg-container">
      <style>{`
        ...
        .msg-section-label {
          ...
          color: hsl(var(--muted-foreground)); padding: 12px 16px 6px;
        }
        ...
        .msg-chat-header {
          ...
          padding: 14px 20px; border-bottom: 1px solid hsl(var(--border));
          background: hsl(var(--card));
        }
        ...
        .msg-online-dot {
          ...
          position: absolute; bottom: 1px; right: 1px; border: 2px solid hsl(var(--card));
        }
        .msg-messages {
        ...
        .msg-bubble {
          max-width: 68%; padding: 9px 14px; border-radius: 18px;
        ...
        .msg-empty {
          ...
          color: hsl(var(--muted-foreground)); padding: 40px; text-align: center;
        }
        .msg-empty-icon {
  ...
  .msg-chat.no-chat { display: none; }
      ...
      {!connected && (
        <div className="msg-ws-banner">Connecting to server…</div>
      )}
              ...
              <div className="msg-messages">
                {broadcasts.length === 0 ? (
                  <div className="msg-empty" style={{ flex: 1 }}>
                    ...
                    <h3 style={{ color: "hsl(var(--foreground))" }}>No broadcasts yet</h3>
                      ...
                      <div key={bc.id}>
                        {showDivider && (
                          <div className="msg-day-divider">
                            <span>{longDateLabel(bc.createdAt)}</span>
                          </div>
                        )}
                        <div className="msg-bubble-group broadcast-msg">
                          ...
                          <div className="msg-bubble-meta">
                            <span>{fmtTime(bc.createdAt)}</span>
                          </div>
                        </div>
                      </div>
                    );
                ...
                {conversation.map((msg, i) => {
                  ...
                  return (
                    <div key={msg.id}>
                      {showDivider && (
                        <div className="msg-day-divider">
                          <span>{longDateLabel(msg.sentAt)}</span>
                        </div>
                      )}
