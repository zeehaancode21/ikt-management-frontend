...
function ProjectDetailsView({ project }) {
  ...
  const copyJobNumber = async () => {
    ...
    try {
      ...
      // Clipboard API can be unavailable/blocked — fail silently, the
      // value is still fully visible and selectable in the card.
    }
  };

  // When the user prefers reduced motion, entrances happen instantly
  // instead of sliding/fading in.
  const sectionInitial = prefersReducedMotion ? "show" : "hidden";
  const gridInitial = prefersReducedMotion ? "show" : "hidden";

  return (
    <div>
      {/* ── Live status strip — the two things people scan for first ── */}
      <motion.div
      ...
      {/* ── Approval — accent color tracks the live status ── */}
      <motion.div
        className="detail-section"
        initial={sectionInitial}
        animate="show"
      ...
      {/* ── FAB — accent color tracks the live status ── */}
      <motion.div
        className="detail-section"
        initial={sectionInitial}
        animate="show"
