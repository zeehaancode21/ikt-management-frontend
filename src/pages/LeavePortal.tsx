...
interface Leave {
  ...
  // Optional note the owner left when rejecting this request.
  rejectionReason?: string | null;
  // Owner's note when a change request was rejected (request stays APPROVED).
  ...
  return (
    ...
    >
      ...
      <div className="space-y-1">
        ...
        {l.status?.toUpperCase() === "REJECTED" && l.rejectionReason && (
          <p className="pl-5 text-xs leading-relaxed text-red-600 dark:text-red-400">
            <span className="font-semibold">Rejection reason:</span> {l.rejectionReason}
          </p>
        )}
        {l.status?.toUpperCase() === "APPROVED" && l.changeRejectionReason && (
          <p className="pl-5 text-xs leading-relaxed text-red-600 dark:text-red-400">
            <span className="font-semibold">Change request rejected:</span> {l.changeRejectionReason}
          </p>
        )}
      </div>
...
const OwnerView = () => {
  ...
  const act = async (id: string | number, action: "APPROVED" | "REJECTED", reason?: string): Promise<boolean> => {
    ...
    try {
      ...
      await api.put(`/leaves/${id}/${action}`, action === "REJECTED" && trimmed ? { reason: trimmed } : undefined);
      toast({
    ...
    <section className="animate-fade-in-up card-hover overflow-hidden rounded-xl border border-border bg-card shadow-sm">
      ...
      <div className="flex flex-col gap-3 border-b border-border/60 px-4 py-4 sm:px-6">
        ...
        <div role="tablist" aria-label="Leave management sections" className="inline-flex w-full items-center rounded-lg border border-border bg-muted/40 p-1 sm:w-auto">
          <TabButton active={ownerTab === "pending"} onClick={() => setOwnerTab("pending")} icon={CalendarClock} label="Due" count={leaves.length} controls={ownerPanelId} />
      ...
      <div id={ownerPanelId} role="tabpanel" className="p-4 sm:p-6">
        ...
        {ownerTab === "pending" && (
          <>
            {loading ? (
              ...
              <div className="space-y-3">
                {leaves.map((l, index) => (
                  <LeaveCard key={l.id} leave={l} index={index} showEmployee onApprove={(leave) => act(leave.id, "APPROVED")} onReject={(leave) => setRejectTarget(leave)} actingId={actingId} />
                ))}
              </div>
            )}
          </>
        ...
        {ownerTab === "employee" && (
          ...
          {rejectTarget && (
        <RejectReasonModal
          title={rejectTarget.status?.toUpperCase() === "REAPPROVAL_PENDING" ? "Reject change request" : "Reject leave request"}
          description={`${rejectTarget.employeeName || "Employee"} · ${fmt(rejectTarget.fromDate)}`}
